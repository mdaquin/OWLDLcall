...
def assertions_entailed_by_calls_by_me(onto_loaded: AbstractReasoner, individual: DLInstance, cache: dict, instances: list[DLInstance]) -> set[PropertyAssertion]:
    """
    Infers assertions from the call formulas of the ontology for a given individual
    ...
    for call in calls:
        if (call, individual) not in cache:
            params_tuples = onto_loaded.list_val_params(individual, call.get_parameters())
            # by assumption of functionality, we don't need to execute on every parameter tuple
            hasParams = False
            for params_tuple in params_tuples:
                hasParams = True
                call.exec(individual, params_tuple, assertions, instances)
...
def infer_calls(onto_iri: str, local_path: str, savefilename: str) -> list[Assertion]:
    ...
    while not end:
        ...
        for i in instances:
            ...
            all_assertions.extend(assertions_entailed_by_calls_by_me(onto_loaded, i, cache, instances))
